...
Полные вычисления: https://docs.google.com/spreadsheets/d/1gPjlxmfjmnKMIx4SSdBtPhAPWj0oFEJtG_SRsOuGHwg/edit?usp=sharing
...
### Правило мутации:
1. Вероятность мутации: 20%
2. Генерируется случайное число от 1 до 10
3. Если число 1 или 2 → инвертируется случайная позиция

### Правило отбора для скрещивания:
1. Сортировка популяции по убыванию приспособленности F(X)
2. Выбираются 2 сильнейшие особи (элита)
3. Применяется двухточечное скрещивание

### Правило формирования нового поколения:
1. Сохраняются 2 самые приспособленные особи (элита)
2. Добавляем потомков, если потомок или потомки неприспособлены, то переходим к следующему шагу
3. Случайным образом разбиваем оставшихся особей на две группы и переносим сильнейших из этих групп в новое поколение
4. Если новое поколение ещё не сформировано, то добираем особей из прошлого по принципу самого приспособленного из оставшихся
...
Инвертируем ячейку 2 у потомка D1 и ячейку 3 у потомка D2
...
В итоге в третье поколения включаются: A5 B1 D2(Мутант) D1(Мутант) B2 A6
...
Из оставшихся A6 и D2(Мутант) - выбираем D2(Мутант) поскольку стоимость выше
